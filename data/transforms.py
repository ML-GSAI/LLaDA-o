...
def shuffle_patch(image, num_splits, gap_size=2):
    """Split the image into patches (allowing indivisible dimensions), shuffle randomly, and concatenate with gaps between patches"""
    ...
    for i in range(h_splits):
        ...
        patch_h = patch_heights[i]  # Height of patches in current row
        for j in range(w_splits):
            # Get the shuffled patch
            patch = patches[patch_idx]
            patch_w = patch_widths[j]  # Width of patches in current column
            # Paste patch (top-left coordinates are (current_x, current_y))
            new_image.paste(patch, (current_x, current_y))
            # Update X coordinate (next patch start position = current patch width + gap)
            current_x += patch_w + gap_size
            patch_idx += 1
        # Update Y coordinate (next row start position = current row height + gap)
...
def inpainting(image, num_splits, blank_ratio=0.3, blank_color=(255, 255, 255)):
    """
    Split image into patches and randomly blank some patches for inpainting task
    
    Args:
        image: PIL.Image input image (RGB mode)
        h_splits: int number of row splits (vertical splits)
        w_splits: int number of column splits (horizontal splits)
        blank_ratio: float ratio of blank patches (0~1)
        blank_color: tuple color of blank region (RGB, e.g., white (255,255,255))
    
    Returns:
        PIL.Image processed concatenated image
    """
    h_splits, w_splits = num_splits
    img_w, img_h = image.size
