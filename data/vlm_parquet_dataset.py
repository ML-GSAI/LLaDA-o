...
class SftVLMParIterableDataset(DistributedIterableDataset):
    def __init__(
    ...
    def change_format(self, data, num_images):
        """
        Convert multiple conversation formats to internal elements list format.
        ...
        If the first user input doesn't contain <image>, add corresponding number of <image> tags at its beginning according to num_images.

        Args:
            data (list): Conversation list, supports dictionaries in multiple formats.
        ...
        for conversation_turn in data:
            # Uniformly extract user and assistant text content
            user_text = None
            assistant_text = None
                            ...
                            for row_idx, row in df.iterrows():
                                ...
                                try: 
                                    ...
                                    if 'images' in row and row['images'] is not None and len(row['images']) > 0:
                                        ...
                                        for image in images:  # images is a list, each image has a bytes field
                                        ...
                                        # elements = self.change_format(row['texts'], 0)
                                        # skip the sample without images
                                        continue
